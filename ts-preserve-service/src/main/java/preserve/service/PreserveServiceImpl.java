...
public class PreserveServiceImpl implements PreserveService {
    ...
    public Response preserve(OrderTicketsInfo oti, HttpHeaders headers) {
        ...
        //走第一条
        if (null == oti.getErrorSceneFlag() || null == oti.getErrorSceneFlag().getTraceWay() || 1 == oti.getErrorSceneFlag().getTraceWay()) {
            ...
            Response result = checkSecurity(oti.getErrorSceneFlag(), oti.getAccountId(), headers);
            ...
            gtdi.setErrorSceneFlag(oti.getErrorSceneFlag());
            ...
            query.setErrorSceneFlag(oti.getErrorSceneFlag());
            query.setDepartureTime(StringUtils.Date2String(new Date()));

            PreserveServiceImpl.LOGGER.info("error scene param {}", JSON.toJSONString(query.getErrorSceneFlag()));
            ...
            order.setErrorSceneFlag(oti.getErrorSceneFlag());
            ...
            //自定义故障场景
            String sceneResult = ErrorUtil.errorScene(oti.getErrorSceneFlag(), ServiceKey.TS_PRESERVE_SERVICE);
            returnResponse.setData(sceneResult);
            return returnResponse;
        } else {

            //security
            checkSecurity(oti.getErrorSceneFlag(), oti.getAccountId(), headers);

            //travel


            String sceneResult = ErrorUtil.errorScene(oti.getErrorSceneFlag(), ServiceKey.TS_PRESERVE_SERVICE);
            Response returnResponse = new Response<>(1, "Success.", "");
            returnResponse.setData(sceneResult);
            return returnResponse;
    ...
    private Response checkSecurity(ErrorSceneFlag errorSceneFlag, String accountId, HttpHeaders httpHeaders) {
        PreserveServiceImpl.LOGGER.info("[checkSecurity][Preserve Other Service][Check Account Security]");

        HttpEntity requestCheckResult = new HttpEntity(errorSceneFlag, httpHeaders);
